...
class Optimizer:
    ...
    def run(self):
        ...
        if self.solver.status in [
            ...
            if (
                self.solver.status == gp.GRB.OPTIMAL
                and self.config.instance_type == "small"
            ):
    ...
    def save_results(self, solution: Solution):
        ...
        instance_results = (
            self.formulation.instance.instance_results
            if self.config.instance_type != "small"
            else {field: None for field in FIELDS_INSTANCE_RESULTS}
        )
        ...
        try:
            results_df.to_csv(
                self.config.results_filepath, index=False, sep=";", decimal=","
            )
        except PermissionError:
            warnings.warn("Could not save results. Saving it with a timestamp.")
            results_df.to_csv(
                f"results_{timestamp}.csv", index=False, sep=";", decimal=","
            )
