...
class InstanceLoader:
    ...
    def __init__(self, config: Config):
        ...
        self.__instance_dir = os.path.join(
            self.__config.data_dir, self.__config.instance_type
        )

        self.__optimal_solutions_small = (
            self.read_optimal_solutions_small()
            if self.__config.instance_type == "small"
            else None
        )
        self.__original_results_large = (
            self.read_original_results_large()
            if self.__config.instance_type == "large"
            else None
        )
        self.__original_results_case = (
            self.read_original_results_case()
            if self.__config.instance_type == "case"
            else None
        )
    ...
    def read_original_results_large(self) -> pd.DataFrame:
        ...
        """
        return pd.read_csv(
            self.__config.large_original_results_file, sep=";", decimal=","
        )
    ...
    def read_original_results_case(self) -> pd.DataFrame:
        ...
        """
        return pd.read_csv(
            self.__config.case_original_results_file, sep=";", decimal=","
        )
        ...
        if self.__config.n_instances_main is not None:
            ...
            if self.__config.n_instances_main == 1 and self.__config.instance_name:
                return {
                    ...
                    if v.name == self.__config.instance_name
                }
            ...
            instances = dict(
                random.sample(instances.items(), self.__config.n_instances_main)
            )
    ...
    def __load_instance(self, instance_file: str) -> Instance:
        ...
        with open(instance_path, "r") as f:
            ...
            if self.__config.instance_type in ["small", "large"]:
                ...
                node_info = [
                    tuple(map(int, line.split())) for line in lines[5 : 5 + N_size + 1]
                ]
                ...
                node_info = [
                    tuple(map(int, line.split())) for line in lines[4 : 4 + N_size + 1]
                ]
                travel_times = [
                    tuple(map(int, line.split()))
                    for line in lines[4 + N_size + 1 : 4 + 2 * (N_size + 1)]
                ]

                t = {
                    (i, j): travel_times[i][j]
                    for i in range(N_size + 1)
                    for j in range(N_size + 1)
                }
                alpha = {
                    (i + 1, c + 1): node_info[i + 1][c]
                    for c in range(C_size)
                    for i in range(N_size)
                }
        ...
        if self.__config.instance_type == "small":
            instance_results = {
                "optimal_value": self.__optimal_solutions_small.get(
                    instance_file[3:-4],
                    KeyError("Instance not found in optimal solutions"),
                )
            }
        elif self.__config.instance_type == "large":
            instance_results = self.get_instance_results_for_large(
                instance_file, N_size, K_size, T_max
            )
        else:  # case
            instance_results = self.get_instance_results_for_case(
                instance_file, C_size, K_size
            )
        ...
        return Instance(
            ...
            t,
            alpha,
            full_name=instance_file,
            instance_results=instance_results,
        )

    def get_instance_results_for_case(
        self, instance_file, C_size, K_size
    ) -> Dict[str, float]:
        """
    ...
    def get_instance_results_for_large(
        self, instance_file, N_size, K_size, T_max
    ) -> Dict[str, float]:
        """
